...
struct ContentView: View {
    ...
    let streamWhite = Color.white
    let streamRed = Color.red
        ...
        ZStack {
            ...
            HeartIcon()
                ...
                .foregroundStyle(.linearGradient(
                    colors: [streamWhite, streamRed], startPoint: .leading, endPoint: .trailing
                )
                )
                .phaseAnimator([false, true]) { openPart, moveAround in
                    HeartIcon()
                        .stroke(style: StrokeStyle(lineWidth: 4, lineCap: .round, lineJoin: .round, miterLimit: 0, dash: [150, 15], dashPhase: moveAround ? -100 : 225))
                        .frame(width: 64, height: 64)
                        .foregroundStyle(.linearGradient(
                            colors: [streamWhite, streamRed], startPoint: .leading, endPoint: .trailing
                        )
